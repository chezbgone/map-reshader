...
def get_palette(schem: Compound) -> dict[int, str]:
    return {
        index: str(entry['Name']) for index, entry in enumerate(schem['palette'])
    }

def get_inverse_palette(schem: Compound) -> dict[str, int]:
    return {
        entry: index for index, entry in get_palette(schem).items()
    }
...
def remove_air(schem: Compound) -> None:
    ...
    blocks_with_no_air = [block for block in schem['blocks'] if block['state'] != air_id]
