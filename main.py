...
def test():
    ...
    optimization_info = pairwise_z3.optimize_monotonic(mapart)
    print(optimization_info)
    print(*mapart.height_map, sep='\n')
...
def optimize_pokemon(
    ...
    with nbtlib.load(f'base_schematics/{pokemon}.nbt') as schem:
        remove_air(schem)
        mapart = ReifiedMapArt.from_nbt_file(schem)
    if strategy == 'pairwise_z3':
        opt_info = pairwise_z3.optimize_monotonic(mapart, scaffolding_kind='minimal')
    else:
    ...
    print(f'  {before_proportion:.4f} -> {after_proportion:.4f} in {opt_info.time_taken.total_seconds():.2f}s')
    regions[pokemon] = mapart.to_region()
    schem = Schematic(pokemon, 'chezbgone', regions=regions).save(f"{pokemon}.litematic")
