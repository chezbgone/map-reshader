...
from staircased import StaircasedMapArt
...
class MapArt:
    ...
    def from_nbt_file(cls, schem: Compound, size: int = 128):
        ...
        for (x, y, z), block_name in NBT.get_blocks(schem):
            if not (0 <= x <= size and 0 <= z < size + 1):
                raise ValueError(f"Block position out of mapart bounds")
            old_block = grid[z][x]
        ...
        pixels = [
            [
                Pixel(block_name, Shading.from_heights(shader_height, block_height))
                    if block_name != "minecraft:air" else
                    None
                for ((_, shader_height), (block_name, block_height)) in zip(shader_row, row)
            ]
    ...
    def _reify_two_layer(self) -> DualLayerMapArt:
        raise ValueError("todo")